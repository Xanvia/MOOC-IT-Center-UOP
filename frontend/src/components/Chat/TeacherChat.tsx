...
const CourseCreatorChat: React.FC<TeacherChatProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await replytoCourseCreator(params.courseId as string, newMessage);
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        ...
        const fetchedMessages = await getMessagesFromAdmin(courseId);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center mb-4 border-b pb-2">
        ...
        <h2 className="text-xl font-semibold">Messages From Course Creator</h2>
      </div>
