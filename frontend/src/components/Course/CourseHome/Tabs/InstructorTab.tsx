      ...
      <div className="flex justify-center lg:mx-32">
        <div className="py-20 px-10 sm:px-20 xl:mx-28 text-left bg-primary_light w-full">
          <figure className="mt-6 text-center">
            <figcaption className="mt-4 flex flex-wrap justify-center gap-20">
              {instructors.map((instructor) => (
                <Link
                  key={instructor.id} // Ensure this uses the correct id
                  href={`/profile/${instructor.id}`} // Pass the correct instructor ID
                >
                  <div className="flex w-80 items-center rounded-lg border bg-white border-gray-200 p-4 shadow-md hover:shadow-lg transition-shadow cursor-pointer">
                    {/* Profile Image */}
